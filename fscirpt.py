...
def main(filepath):
    ...
    month = df.columns[0].capitalize().strip()
    ...
    for url in urls:
        ...
        if len(data) == 0:
            ...
            outdf.loc[urls.index(url), "TIME STAMP"] = now.strptime(now.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S")
            outdf.loc[urls.index(url), "SECTOR"] = df.loc[urls.index(url) + 1,"sectr"]
            outdf.loc[urls.index(url), 'SYMB'] = df.loc[urls.index(url) + 1,"symb"]
            if df.loc[urls.index(url) + 1, "LUU"] == "0":
                outdf.loc[urls.index(url), "LUU"] = 0
            else:
            ...
            outdf.loc[urls.index(url), "Previous Available Quarter"] = "NONE"
        elif len(data) > 0:
            ...
            if len(dg.columns) > 1 and check == 1:
                print(df.loc[urls.index(url) + 1, "symb"])
                outdf.loc[urls.index(url),'SYMB'] = df.loc[urls.index(url) + 1, "symb"]
                ...
                outdf.loc[urls.index(url),"SECTOR"] = df.loc[urls.index(url) + 1, "sectr"]
                if gc == 1:
                    outdf.loc[urls.index(url),"Previous Available Quarter"] = prevQ
                elif gc == 0:
                    outdf.loc[urls.index(url),"Previous Available Quarter"] = "NONE"
                if dg.loc[1][1] == 0 and (dg.loc[28][1]):
                    outdf.loc[urls.index(url), f"{month}_R"] = 0.01
                    outdf.loc[urls.index(url),f"{month}_V"] = convert(dg.loc[28][1])
                    outdf.loc[urls.index(url),f"{month}_DEPS"] = convert(dg.loc[37][1])
                    outdf.loc[urls.index(url),f"{month}_Interest"] = convert(dg.loc[20][1])
                elif "bank" in outdf.loc[urls.index(url), "SECTOR"].lower():
                    print("bank")
                    b = 1
                elif b == 0:
                    outdf.loc[urls.index(url),f"{month}_R"] = convert(dg.loc[1][1])
                    outdf.loc[urls.index(url),f"{month}_V"] = convert(dg.loc[28][1])
                    outdf.loc[urls.index(url),f"{month}_DEPS"] = convert(dg.loc[37][1])
                outdf.loc[urls.index(url),f"{month}_Interest"] = convert(dg.loc[20][1])
                ...
                if "bank" in outdf.loc[urls.index(url), "SECTOR"].lower():
                    outdf.loc[urls.index(url),f"{month}_R"] = convert(dg.loc[2][1])
                    outdf.loc[urls.index(url),f"{month}_V"] = convert(dg.loc[20][1])
                    outdf.loc[urls.index(url),f"{month}_DEPS"] = convert(dg.loc[29][1])
                    outdf.loc[urls.index(url), f"{month}_Interest"] = 0
                    outdf.loc[urls.index(url),f"{month}_Gross NPA"] = convert(dg.loc[35][1])
                    outdf.loc[urls.index(url),f"{month}_Net NPA"] = convert(dg.loc[36][1])
            else:
                outdf.loc[urls.index(url),"RES_URL"] = df.loc[urls.index(url) + 1, "res_url"]
                outdf.loc[urls.index(url), "DATA_AVAILABLE"] = "N"
                outdf.loc[urls.index(url), "TIME STAMP"] = now.strptime(now.strftime("%Y-%m-%d %H:%M:%S"), "%Y-%m-%d %H:%M:%S")
